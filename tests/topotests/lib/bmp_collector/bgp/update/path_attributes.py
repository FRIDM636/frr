...
class PathAttrAsPath:
    ...
    def get_asn_len(asns):
        """shitttty function to determine asn length"""
        pass
...
class PathAttrMpReachNLRI:
    ...
    @classmethod
    def dissect_nlri(cls, nlri_data, afi, safi):
        addr_family = AddressFamily(afi, safi)
        if addr_family == AF.IPv6_VPN:
            return NLRI.NlriIPv6Vpn.parse(nlri_data)

        elif addr_family == AF.IPv4_VPN:
            return NLRI.NlriIPv4Vpn.parse(nlri_data)

        elif addr_family == AF.IPv6_UNICAST:
            return NLRI.NlriIPv6Unicast.parse(nlri_data)

        # XXX add the rest of address families
    ...
    def dissect(cls, data):
        ...
        offset += 1
        # dissect nlri
        nlri = cls.dissect_nlri(data[offset:], afi, safi)

        return {**nexthop, **{'afi': afi, 'safi': safi,'ip_prefix': nlri}}
...
class PathAttrMpUnReachNLRI:
    ...
    def dissect(cls, data):
        ...
        # have we withdrawn routes?
        if data[offset:]:
            # dissect withdrawn_routes
            withdrawn_routes = NLRI.dissect_nlri(data[offset:], afi, safi)

            return {
                "bmp_log_type": "withdraw",
                'afi': afi,
                'safi': safi,
                'ip_prefix': withdrawn_routes,
            }
